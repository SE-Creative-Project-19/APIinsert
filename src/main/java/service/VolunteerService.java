...
public class VolunteerService{
    ...
    public List<UserDTO> getVolunteerListByServicePK(List<VolunteerDTO> volunteerDTOS, int pageNo){ // 봉사 신청 리스트에서  신청자에 대한  userDTO를 리스트 return
       List<UserDTO> userDTOS = userService.getUsersByPK(volunteerDTOS, pageNo);
       return userDTOS;
    }
    public List<VolunteerDTO> getVolunteerApplicant(String facility) {// 담당자가 해당 기관의 승인이 필요한 봉사 신청 리스트를 return
    ...
    public List<VolunteerDTO> getVolunteerDone(String facility) { // 봉사기간이 지나서 봉사는 완료되었으나 별점을 등록하지 않은 봉사 신청 리스트를 return
